...
speed_gauge = tk_tools.Gauge(root, height=180, width=320,
                             ...
                             divisions=10, red=60, yellow=40)
speed_gauge.grid(row=0, column=0, sticky='news')

tach_gauge = tk_tools.Gauge(root, height=180, width=320,
                            max_value=8000,
                            label='Taco Metri', unit=' RPM',
                            divisions=10, red=65, yellow=60)
tach_gauge.grid(row=1, column=0, sticky='news')

boostT2_gauge = tk_tools.Gauge(root, height=180, width=320,
                               max_value=2.5,
                               label='boost T2', unit=' Bar',
                               divisions=10, red=90, yellow=75, red_low=15, yellow_low=20)
boostT2_gauge.grid(row=2, column=0, sticky='news')

boostT1_gauge = tk_tools.Gauge(root, height=180, width=320,
                               max_value=7,
                               label='boost T1', unit=' Bar',
                               divisions=10, red=95, yellow=80, red_low=30, yellow_low=35)
boostT1_gauge.grid(row=2, column=1, sticky='news')
...
batI_gauge = tk_tools.Gauge(root, height=180, width=300,
                            ...
                            label='Tps', unit='%',
                            divisions=14, yellow=105, red=110)
...
def update_gauge():
    ...
    boostT1_gauge.set_value(dataa1[1]/100)
    boostT2_gauge.set_value(dataa1[2]/100)
